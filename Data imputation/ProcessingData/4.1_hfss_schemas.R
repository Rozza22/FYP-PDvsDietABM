...
# write.csv(data, "Data/NDNS/processed_fooddiary.csv", row.names=F)

# wd <- ("C:/Users/ruair/OneDrive - sheffield.ac.uk/Fourth Year/Project/")
wd <- ("//wsl.localhost/Ubuntu/home/ruairi/ABM_code/abmcodeupdated")
setwd(wd)

data <- read_csv("Data imputation/Data/NDNS/processed_fooddiary.csv")
clusters <- read_csv("Data imputation/Data/NDNS/final_clusters.csv")
data <- left_join(data, clusters)

NDNS <- read_csv("Data imputation/Data/NDNS/NDNS_combined.csv") %>% # Which now includes GHQ-12 score
  ...
  dplyr::select(seriali, serialh, age, Sex, ethgrp5, eth_new, nssec8, SES, bmival, ghq12scr) # now have to keep this ghq12scr on all the rest of them
# dplyr::select(time, age, Sex, ethgrp5, eth_new, nssec8, SES, bmival, ghq12scr) # now have to keep this ghq12scr on all the rest of them
...
clusters <- data %>% 
  ...
  dplyr::select(SurveyYear, DayNo, seriali, age, Sex, eth_new, nssec8, bmival, ghq12scr,
...
summary <- clusters %>% ungroup() %>% 
  ...
  mutate(proportion=TotalKcal/sum(TotalKcal)) # Could put GHQ12 in here to visualise how it is related
...
HFSS <- data %>% mutate(HFSS = ifelse(description=="biscuits, chocolate, crisps, puddings", 1,0)) %>% 
  mutate(agecat = cut(age, breaks=c(0,15,24,34,44,54,64,100),
                      labels=c("0-15","16-24","25-34","35-44","45-54",
                               "55-64","65+"))) %>% 
  group_by(seriali, serialh, SurveyYear, DayNo, age, agecat, Sex, ethgrp5, eth_new,  bmival, ghq12scr, nssec8, SES, HFSS) %>% 
...
ggsave("Data imputation/Data/NDNS_outputs/plot_distribution_HFSS_kcal_race.png", dpi=300, width=33,
       height=19, units="cm")

HFSS <- HFSS %>% group_by(Sex, eth_new, SES) %>%  # Can make a plot where we focus only on ghq12scr
  mutate(median = median(TotalKcal)) %>% 
  # filter(agecat!="0-17") %>% 
...
ggsave("Data imputation/Data/NDNS_outputs/plot_distribution_HFSS_kcal_SES.png", dpi=300, width=33,
       height=19, units="cm")
...
ggsave("Data imputation/Data/NDNS_outputs/plot_distribution_HFSS_kcal_age_SES.png", dpi=300, width=33,
       height=19, units="cm")

write.csv(HFSS, "Data imputation/Data/NDNS_outputs/NDNS_HFSS_food_diaries.csv", row.names=F)
...
individualdata <- HFSS %>% #proportion each individual gets from each category - for initialisation of model 
  ...
  group_by(seriali, serialh, SurveyYear, age, agecat, Sex,ethgrp5, eth_new,nssec8, SES, bmival, ghq12scr, HFSS, KcalBin) %>% 
  summarise(TotalKcal = sum(TotalKcal)) %>% ungroup() %>% 
  group_by(seriali, serialh, SurveyYear, age, agecat, Sex, ethgrp5, eth_new, nssec8, SES, bmival, ghq12scr, HFSS) %>% 
...
otherfoods <- HFSS %>% 
  # filter(HFSS==0) %>% 
  group_by(seriali, DayNo, HFSS, age, Sex, eth_new, ethgrp5, nssec8, SES, bmival, ghq12scr) %>% 
  summarise(TotalKcal = sum(TotalKcal)) %>% 
  ungroup() %>%
  group_by(seriali, HFSS, age, Sex, eth_new, ethgrp5, nssec8, SES, bmival, ghq12scr) %>%
...
write.csv(individualdata, "Data imputation/Data/NDNS_outputs/NDNS_schema_HFSS.csv", row.names=F)
...
write.csv(socialnorms, "Data imputation/Data/NDNS_outputs/NDNS_descriptive_norms.csv", row.names=F)
...
ggsave("Data imputation/Data/NDNS_outputs/targets_Sex_SES.png", dpi=300, width=33,
       height=19, units="cm")
