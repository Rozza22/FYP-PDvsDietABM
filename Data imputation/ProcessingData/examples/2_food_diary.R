...
for(i in 1:length(nutrientprofile)){
  ...
  nutrientprofile[[i]] <- nutrientprofile[[i]] %>% 
    ...
    dplyr::select(SurveyYear, FoodName, FoodNumber, KCALS, KJ, PROT, FAT, CHO, TOTSUG,
...
summary(data$Total_NP_score)
# try a hierarchical cluster
library(stats)

HFSS <- data %>% filter(HFSS == 1)
...
unique(HFSS$SubFoodGroupDesc)

# perform clustering on non HFSS foods

maingroups <- data %>% ungroup() %>% 
  # filter(HFSS==0) %>% 
  dplyr::select(MainFoodGroupDesc, KCALS,
                                                   PROT, FAT, CHO, TOTSUG, SATFA, `NA`, ENGFIB,
                HFSS, Mealtime,
                                                   FruitVegNut, Total_NP_score) %>% 
  pivot_longer(cols=KCALS:Total_NP_score) %>% 
  ...
  # unnest() %>% 
  dplyr::select(MainFoodGroupDesc, CHO:TOTSUG) %>% distinct()
...
library(factoextra)
res.hc <- eclust(clusterdata, "hclust", nboot=500, k=10) # compute hclust
# fviz_dend(res.hc, rect = TRUE) # dendrogam
# fviz_silhouette(res.hc)
plot_clusters <- fviz_cluster(res.hc) + theme_bw() # scatter plot
plot_clusters
ggsave("Data/NDNS/cluster_plot_v2.png", dpi=300, width=33, height=19, units="cm")
...
clusterlabels <- data.frame(cluster=1:7, label=c("milk, potatoes, pasta, rice, soft drinks, ice cream",
                                                 "meats and fish",
                                                 "cakes, biscuits, sugars, crisps, savoury snacks",
                                                 "breads, cereals, nuts and seeds",
                                                 "butter, fats, spreads",
                                                 "fruit, veg, salads",
                                                 "miscellaneous"))

data <- left_join(data, clusterlabels)
summarypop <- data %>% group_by(SurveyYear, label) %>% 
  summarise(TotalKcal = sum(Energykcal)) %>% ungroup() %>% 
  group_by(SurveyYear) %>% 
  mutate(percent = TotalKcal / sum(TotalKcal))
scaleFUN <- function(x) sprintf("%.2f", x)

summary_plot <- ggplot(data=summarypop, aes(x=SurveyYear, y=percent, colour=as.factor(label))) + geom_line(size=1) +
  theme_bw() + theme(legend.title=element_blank(),
                     legend.position="bottom") +
  scale_y_continuous(labels=scaleFUN)

summary_plot
ggsave("Data/NDNS/cluster_prevalence_v2.png", dpi=300, width=33, height=19, units="cm")
