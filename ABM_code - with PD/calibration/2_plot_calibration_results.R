...
targets <- read.csv("calibration/targetDataActual.csv")
targets %>% 
...
names <- (Sys.glob("outputs/calibration/annual_data_props_calibration*.csv")) # reads in files that start with calibration and end with .csv
# names <- (Sys.glob("outputs/calibrationNew/annual_data_props_calibration*.csv")) # reads in files that start with calibration and end with .csv

# names <- (Sys.glob("outputs/calibration/yaml_2*.csv"))
# names <- names[1:100]
files <- lapply(names, read.csv)

names <- gsub("/outputs/calibration/annual_data_props_calibration_model_", "", names)
# names <- gsub("/outputs/calibrationNew/annual_data_props_calibration_model_", "", names)
...
ggsave("calibration/plots/PHASE_full_calibration.png", dpi=300, width=33, height=19,
       units="cm")
...
# bestHyperparameter <- read_yaml("/props/calibration/model_", "", as.integer(ids))

# Construct the file path
file_path <- paste0("props/calibration/model_", ids, ".yaml")

# Read the YAML file
bestHyperparameters <- read_yaml(file_path)
write_yaml(bestHyperparameters, file = "props/calibration/best_hyperparameters.yaml")
